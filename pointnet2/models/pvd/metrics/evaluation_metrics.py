...
def EMD_CD(sample_pcs, ref_pcs, f1_threshold = 0.001):
    N_sample = sample_pcs.shape[0]
    N_ref = ref_pcs.shape[0]
    assert N_sample == N_ref, "REF:%d SMP:%d" % (N_ref, N_sample)
    dl, dr, _, _ = cham3D(sample_pcs, ref_pcs)
    fs = fscore(dl, dr, threshold = f1_threshold)[0] #.cpu()
    cd = dl.mean(dim=1) + dr.mean(dim=1)
    emd_dist = EMD(sample_pcs, ref_pcs, transpose=False)
