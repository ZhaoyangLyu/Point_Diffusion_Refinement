...
import sys
sys.path.append('../shapenet_dataloader')
...
def distChamferCUDA(x, y):
    distance, _ = chamfer_distance(x, y, batch_reduction=None)
...
def emd_approx(sample, ref):
    ...
    emd_norm = earth_mover_distance(sample, ref, transpose=False, return_match=False)
    return emd_norm
...
def _pairwise_EMD_CD_(sample_pcs, ref_pcs, batch_size, accelerated_cd=True, report_interval=10):
    N_sample = sample_pcs.shape[0]
    N_ref = ref_pcs.shape[0]
    all_cd = []
    all_emd = []
    ...
    for sample_b_start in iterator:
        if sample_b_start % report_interval == 0:
            print('pairwise_EMD_CD computation progress %.4f' % (sample_b_start/N_sample))
...
def knn(Mxx, Mxy, Myy, k, sqrt=False):
    # Mxx is distance matrix for ref point clouds
    # Mxy is distance matrix for ref and sample point clouds
    # Myy is distance matrix for sample point clouds
    n0 = Mxx.size(0)
    n1 = Myy.size(0)
...
def compute_all_metrics(sample_pcs, ref_pcs, batch_size, accelerated_cd=False):
    ...
    print('Computing pairwise_EMD_CD between sample_pcs and ref_pcs', flush=True)
    ...
    print('Computing pairwise_EMD_CD between ref_pcs and ref_pcs', flush=True)
    M_rr_cd, M_rr_emd = _pairwise_EMD_CD_(ref_pcs, ref_pcs, batch_size, accelerated_cd=accelerated_cd)
    print('Computing pairwise_EMD_CD between sample_pcs and sample_pcs', flush=True)
...
if __name__ == "__main__":
    ...
    # emd_norm = emd_approx(x, y)
    # pdb.set_trace()
    with torch.no_grad():
        sample_pcs = torch.rand(B, N, 3).cuda()
        ref_pcs = torch.rand(B, N, 3).cuda()
        batch_size = 256
        ...
        pdb.set_trace()
        for key in results:
            # print('{}\t{}'.format(key, results[key].item()))
